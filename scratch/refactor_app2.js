...
`;
code = code.replace(/function escapeHtml\(str\) \{/, match => apiFetchCode + '\n' + match);

// 2. Fix GoogleAuthManager to persist token
code = code.replace(/login\(userObj, idToken = null\) \{[\s\S]*?particleInstance\.burst\(\);\s*\}/, `login(userObj, idToken = null) {
    this.user = userObj;
...
code = code.replace(/loadUser\(\) \{[\s\S]*?\}\s*\}/, `loadUser() {
...
// We will replace the entire save methods so they do nothing.
code = code.replace(/saveBalance\(\) \{[\s\S]*?\}\s*saveInventory/g, `saveBalance() { /* handled by server */ }\n  saveInventory`);
code = code.replace(/saveInventory\(\) \{[\s\S]*?\}\s*saveVault/g, `saveInventory() { /* handled by server */ }\n  saveVault`);
code = code.replace(/saveVault\(\) \{[\s\S]*?\}\s*saveStats/g, `saveVault() { /* handled by server */ }\n  saveStats`);
code = code.replace(/saveBoosters\(\) \{[\s\S]*?\}\s*addHistory/g, `saveBoosters() { /* handled by server */ }\n  addHistory`);
...
code = code.replace(/async function handleUpgradeClick\(e\) \{[\s\S]*?finishUpgrade\(isWin, roll, chance\);\s*\}\s*\)\s*;\s*\}/, `
...
// 5. Booster buying
code = code.replace(/function handleBuyBooster\(type\) \{[\s\S]*?showToast\('Бустер активовано!', 'success'\);\s*\}/, `
...
// 6. Sell Vault Item
code = code.replace(/function sellVaultItem\(item\) \{[\s\S]*?updateUi\(\);\s*\}/, `
function sellVaultItem(item) {
  ...
  apiFetch('/api/game/sell', {
    ...
    if (state.vault.length === 0) {
      ...
      // Re-open with new state or just let updateUi handle it if vault withdraw modal has render function
